...
> **状态**：提案（2026-09-01，待产品决策，未进入实现）。
> **适用范围**：Desktop 优先；Mobile / 远程场景见 §9。
...
- **新增 IPC 通道**（`apps/desktop/src/main/maker-ipc/channels.ts`，main 侧在
  ...
  (soft) / restore / trash list / history / search / prompt preview / review run /
  consolidate preview / reset snapshot export。Renderer 不接触文件路径。
...
| P3 维护产品化 | 重置快照、体检卡片、合并预览 | 重置可回滚；体检建议可逐条采纳；合并前有预览 |
| P4（可选） | Mobile / 远程只读记忆摘要 | 见 §9 |
...
- Mobile / 远程连接不复制编辑界面；P4 只提供当前 workdir 的只读摘要
  （L1 子集），编辑请求回落到桌面端处理，符合「每端承担适合它的职责」。
...
- 不做跨 agent 记忆编辑器（Claude / Codex 原生记忆仍由各自体系管理，
  记忆中心只管 Cindy 共享记忆）。
- 不自动执行任何 LLM 体检 / 合并；一切 LLM 动作显式触发。

## 11. 待决策事项

1. 命名：界面名称候选「记忆中心」/「记忆管理」，以术语表裁决为准。
2. 重置快照保留份数（建议 3）与保留时长。
3. 事件日志保留上限（建议 500 条或 2MB，先到为准）。
4. 体检是否允许「一键全部采纳」（涉及批量写入，建议首期只支持逐条）。
5. P1 是否顺带展示 per-agent 原生记忆（Claude / Codex）的存在状态（建议
   首期不展示，避免与共享记忆混淆）。
